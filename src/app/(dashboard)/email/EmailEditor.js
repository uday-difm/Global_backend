...
export default function EmailEditor({ siteId, initialEmailSettings }) {
  ...
  const [username, setUsername] = useState(initialEmailSettings?.username || "");
  const [password, setPassword] = useState(initialEmailSettings?.password || "");
  const [formEmail, setFormEmail] = useState(initialEmailSettings?.formEmail || "");
  
  // Auto Reply & Alerts States
  const [autoReplyEnabled, setAutoReplyEnabled] = useState(!!initialEmailSettings?.autoReplyTemplate?.enabled);
  const [autoReplySubject, setAutoReplySubject] = useState(initialEmailSettings?.autoReplyTemplate?.subject || "");
  const [autoReplyBody, setAutoReplyBody] = useState(initialEmailSettings?.autoReplyTemplate?.body || "");

  const [adminAlertsEnabled, setAdminAlertsEnabled] = useState(!!initialEmailSettings?.adminAlerts?.enabled);
  const [adminAlertsEmail, setAdminAlertsEmail] = useState(initialEmailSettings?.adminAlerts?.email || "");

  // Logs state
  const [failedLogs, setFailedLogs] = useState(initialEmailSettings?.failedLogs || []);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Email configurations and templates updated successfully!" });
  ...
  const handleTestSmtp = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || data.error || "SMTP verification failed");
      }
      ...
      setMessage({ type: "error", text: `SMTP Test Failed: ${err.message}. Please save your correct settings first.` });
  ...
  const handleClearLogs = async () => {
    if (!confirm("Are you sure you want to purge all failed email logs?")) return;
    ...
    try {
      ...
      setFailedLogs([]);
      setMessage({ type: "success", text: "Failed email logs cleared successfully" });
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6">
      {/* Action Messages */}
      {message && (
        <div className={`p-4 border-l-4 rounded-lg flex items-start gap-3 shadow-xs animate-in fade-in duration-200 ${
          ...
          <div>
            <p className="font-semibold text-xs uppercase tracking-wider">{message.type === "success" ? "Success" : "Configuration Alert"}</p>
        ...
        <div className="p-6 flex-1">
          ...
          {activeTab === "smtp" && (
            <div className="space-y-6">
              <div className="border-b border-gray-100 pb-3 flex justify-between items-center flex-wrap gap-3">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">SMTP Server Settings</h3>
                  <p className="text-xs text-gray-500 mt-1">Setup your transactional outgoing email server credentials.</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2">
                  <label htmlFor="smtp_host" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                ...
                {/* SMTP Port */}
                <div>
                  <label htmlFor="smtp_port" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                    SMTP Port
                  </label>
                  <input
                    type="text"
                ...
                {/* Username */}
                <div>
                  <label htmlFor="smtp_user" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                    SMTP Username / Login
                  </label>
                  <input
                    type="text"
                ...
                {/* Password */}
                <div>
                  <label htmlFor="smtp_pass" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                    SMTP Password
                  </label>
                  <input
                    type="password"
                ...
                {/* Sender Email */}
                <div>
                  <label htmlFor="form_email" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                    Sender / "From" Email
                  </label>
                  <input
                    type="email"
              ...
              <div className="bg-gray-50/50 border border-gray-200 rounded-xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b pb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Form Email Auto-Reply</h4>
                      <p className="text-[10px] text-gray-500">Auto-respond to visitors when they submit contact form inquiries.</p>
                    </div>
                  </div>
                ...
                {autoReplyEnabled && (
                  <div className="space-y-3.5 animate-in fade-in slide-in-from-top-1 duration-150">
                    <div>
                      <label htmlFor="reply_subject" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 flex justify-between">
                        <span>Email Subject Line</span>
                        <span className="normal-case text-[9px] font-normal text-gray-400">Use {"{name}"} for customer name</span>
                      </label>
                      <input
                        type="text"
                        id="reply_subject"
                    ...
                    <div>
                      <label htmlFor="reply_body" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 flex justify-between">
                        <span>Email Body Message</span>
                        <span className="normal-case text-[9px] font-normal text-gray-400">Placeholders: {"{name}"}, {"{siteName}"}</span>
                      </label>
                      <textarea
                        id="reply_body"
                        value={autoReplyBody}
              ...
              <div className="bg-gray-50/50 border border-gray-200 rounded-xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b pb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-gray-800 uppercase tracking-wider">Admin Notification Alerts</h4>
                      <p className="text-[10px] text-gray-500">Alert administrators instantly when leads register or submit contact forms.</p>
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                {adminAlertsEnabled && (
                  <div className="animate-in fade-in slide-in-from-top-1 duration-150">
                    <label htmlFor="admin_email" className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">
          ...
          {activeTab === "logs" && (
            <div className="space-y-6">
              <div className="border-b border-gray-100 pb-3 flex justify-between items-center flex-wrap gap-3">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">Failed Email Delivery Logs</h3>
                  <p className="text-xs text-gray-500 mt-1">Review SMTP connection issues and alert failures (last 50 logs).</p>
                </div>
                  ...
                  <tbody className="divide-y divide-gray-150">
                    {failedLogs.map((log, idx) => (
                      <tr key={idx} className="hover:bg-white transition-colors">
                        ...
                        <td className="p-3 font-semibold text-gray-700 font-mono">{log.to || "N/A"}</td>
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded text-[8px] font-bold border uppercase ${
                            log.context === "admin-alert" ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-blue-50 text-blue-600 border-blue-200"
                          }`}>
                            {log.context || "smtp-test"}
                          </span>
                        </td>
                        <td className="p-3 text-red-600 font-mono leading-relaxed select-all break-all">{log.error}</td>
                      </tr>
                    ))}
                    {failedLogs.length === 0 && (
                      <tr>
                        <td colSpan="4" className="text-center py-12 text-gray-400 italic">
                          <div className="flex flex-col items-center gap-1.5">
                            ...
                            <span>No SMTP transmission failures found. Outbox deliveries are running cleanly!</span>
                          </div>
                        </td>
                      </tr>
                    )}
